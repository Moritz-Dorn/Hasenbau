...
2. **Deterministisches vor Probabilistischem.** Bevor ein Hase Material
   sieht, laufen Gänge — normale Skripte ohne LLM. Der Hase bekommt
...
3. **Verdichtung.** Ein Hase, der bei jedem Lauf dieselben Tool-Calls
   ...
   loggt die Traces; `hasenbau dig` und der Baumeister machen daraus
...
Auftrags. Das ist auch der Grund, warum `hasenbau init` einen
Root-Commit anlegt: ohne ihn bekommt der Bau keine eigene Projekt-ID,
und die Raum-Permissions greifen nicht.
...
und schreibt sie nie automatisch, sondern zeigt erst den Diff.
...
Gestoppt wird mit `systemctl --user stop hasenbau` — das ist `SIGTERM`
und damit derselbe saubere Weg wie Ctrl-C im Vordergrund.

## Zwei Datenbanken

Die SQLite unter `state/hasenbau.db` trackt, was der Hasenbau *tut*
(PLAN.md §5). Beads trackt, wie der Hasenbau *gebaut* wird — das ist die
Entwicklung dieses Repos und hat mit einem laufenden Bau nichts zu tun.
