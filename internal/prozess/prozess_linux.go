...
const startZeitVerfuegbar = true

// tickProSekunde ist USER_HZ: /proc/<pid>/stat rechnet in diesen Ticks.
...
const tickProSekunde = 100

// bootZeit ist der Nullpunkt für Feld 22 aus /proc/<pid>/stat und
// ändert sich über die Prozess-Lebensdauer nicht.
var bootZeit = sync.OnceValues(func() (time.Time, bool) {
...
// startZeit liest die Startzeit des Prozesses aus /proc/<pid>/stat
...
func startZeit(pid int) (time.Time, bool) {
	...
	return boot.Add(time.Duration(ticks) * time.Second / tickProSekunde), true
}
