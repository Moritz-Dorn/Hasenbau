...
func Lade(root, name string) (*Template, error) {
	...
	if t.Prompt == "" {
		return nil, fehler("prompt missing — the Markdown part is the system prompt")
	}
...
func loadKnowledge(root string, knowsHasenbau bool, muster []string) ([]Knowledge, error) {
	...
	for _, m := range muster {
		...
		if len(treffer) == 0 {
			return nil, fmt.Errorf("wissen %q: no file found", m)
		}
...
func parseDenies(node *yaml.Node) ([]Regel, error) {
	...
	if node.Kind != yaml.MappingNode {
		return nil, fmt.Errorf("permission has to be a mapping")
	}
	var denies []Regel
	for i := 0; i+1 < len(node.Content); i += 2 {
		...
		case yaml.ScalarNode:
			if wert.Value != "deny" {
				return nil, fmt.Errorf("permission %s: %q — templates may only set deny, allow/ask come from the Auftrag", perm, wert.Value)
			}
		...
		case yaml.MappingNode:
			for j := 0; j+1 < len(wert.Content); j += 2 {
				...
				if aktion != "deny" {
					return nil, fmt.Errorf("permission %s[%s]: %q — templates may only set deny, allow/ask come from the Auftrag", perm, pattern, aktion)
				}
				denies = append(denies, Regel{Permission: perm, Pattern: pattern})
			}
		default:
			return nil, fmt.Errorf("permission %s: expected deny or a pattern mapping", perm)
		}
	}
	return denies, nil
}
...
func Generiere(a *auftrag.Auftrag, t *Template, o Optionen) ([]byte, error) {
	if a.Hase != t.Name {
		return nil, fmt.Errorf("Auftrag %s asks for Hase %q, template is named %q", a.Name, a.Hase, t.Name)
	}
	...
	if len(freigegeben) > 0 {
		...
		return nil, fmt.Errorf("Auftrag %s: tools: %s — no such tool in the Bau (tools/<name>.json)",
	...
	b.WriteString("---\n")
	fmt.Fprintf(&b, "# GENERATED by hasenbau from hasen/%s.md + auftraege/%s.md — do not edit by hand.\n", t.Name, a.Name)
	beschreibung := t.Description
	if beschreibung == "" {
		beschreibung = fmt.Sprintf("Hase %s for Auftrag %s", t.Name, a.Name)
	}
